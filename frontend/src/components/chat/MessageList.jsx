...
function MessageBubble({ msg, isMine, isFirst, onEdit, onDelete }) {
    ...
    // sync text if message updated externally
    useEffect(() => {
    ...
    // focus + cursor to end when editing starts
    useEffect(() => {
    ...
    return (
        <div className={`flex flex-col ${isMine ? "items-end" : "items-start"} ${isFirst ? "mt-3" : "mt-0.5"}`}>
            <div className={`flex items-end gap-2 ${isMine ? "flex-row-reverse" : "flex-row"}`}>
                ...
                {!isMine && (
                    <div className={`w-7 h-7 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 text-white flex items-center justify-center text-xs font-bold uppercase flex-shrink-0 mb-0.5 ${isFirst ? "opacity-100" : "opacity-0"}`}>
                ...
                <div className="flex flex-col max-w-sm">
                    <div
                        className={`relative px-4 py-2.5 text-sm leading-relaxed shadow-sm cursor-default
                            ${isMine
                        ...
                        {isMine && !editing && (
                            ...
                            >
                                <button
                                    ...
                                    className="w-6 h-6 flex items-center justify-center rounded-full bg-white shadow-md border border-gray-200 text-gray-500 hover:text-indigo-500 hover:border-indigo-300 transition"
                                >
                                ...
                                {menuOpen && (
                                    <div className="absolute right-0 top-7 w-32 bg-white rounded-xl shadow-xl border border-gray-100 py-1 z-50">
                                        <button
                                            onClick={() => { setEditing(true); setMenuOpen(false); setHovered(false); }}
                                        ...
                                        <button
                                            onClick={() => { onDelete(msg._id); setMenuOpen(false); setHovered(false); }}
            ...
            {editing && (
                <div className="mt-1.5 w-full max-w-sm">
                    ...
                    <p className="text-[10px] text-gray-400 mt-1 px-1 text-right">Enter to save · Esc to cancel</p>
                </div>
            )}
        </div>
    );
